...
template <typename T> class InvSphere : public Hittable<T> {
public:
  InvSphere(const Point3<T> &center, T radius, shared_ptr<Material> mat)
  ...
  bool hit(const Ray<T> &r, T ray_tmin, T ray_tmax,
    ...
    Vec3<T> inward_normal = (m_center - h_record.m_hitPoint) / m_radius;
    h_record.set_faceNormal(r, inward_normal);
